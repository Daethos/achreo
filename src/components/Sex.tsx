...
export default function Sex({ newAscean, setNewAscean }: { newAscean: Accessor<CharacterSheet>, setNewAscean: Setter<CharacterSheet> }) {
    ...
    return <div class="center creature-heading fadeIn" style={{ "margin-bottom": "3%" }}>
        <For each={sexes}>
            {(sex) => (
                <button class="highlight" onClick={() => {setNewAscean({ ...newAscean(), sex: sex.name }); click.play();}} style={{ color: newAscean().sex === sex.name ? "gold" : "#fdf6d8", animation: newAscean().sex === sex.name ? "flicker 1s infinite ease alternate" : "none" }}>{sex.name}</button>
            )}
        </For>
