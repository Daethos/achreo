...
export default function CombatText({ combat, combatHistory, logs }: { combat: Accessor<Combat>, combatHistory: Accessor<string>, logs: HTMLElement }) {
    const dimensions = useResizeListener();
    // const [combatHistory, setCombatHistory] = createSignal<any>(undefined);
    const text = (prev: string) => {
        ...
        //         {prev !== undefined && <p>{result}</p>}
        //         {combat().playerStartDescription && <p>{combat().playerStartDescription}</p>}
        //         {combat().computerStartDescription && <p>{combat().computerStartDescription}</p>}
        //         {combat().playerSpecialDescription && <p>{combat().playerSpecialDescription}</p>}
        //         {combat().computerSpecialDescription && <p>{combat().computerSpecialDescription}</p>}
        //         {combat().playerActionDescription && <p>{combat().playerActionDescription}</p>}
        //         {combat().computerActionDescription && <p>{combat().computerActionDescription}</p>}
        //         {combat().playerInfluenceDescription && <p>{combat().playerInfluenceDescription}</p>}
        //         {combat().playerInfluenceDescriptionTwo && <p>{combat().playerInfluenceDescriptionTwo}</p>}
        //         {combat().computerInfluenceDescription && <p>{combat().computerInfluenceDescription}</p>}
        //         {combat().computerInfluenceDescriptionTwo && <p>{combat().computerInfluenceDescriptionTwo}</p>}
        //         {combat().playerDeathDescription && <p>{combat().playerDeathDescription}</p>}
        //         {combat().computerDeathDescription && <p>{combat().computerDeathDescription}</p>}
    ...
    const history = createMemo((prev) => text(prev as unknown as string));

    return (
        <div style={dimensions().ORIENTATION === 'landscape' ? { 
            position: 'absolute', left: '25vw', height: '30vh', width: '50vw', top: '60vh', margin: 'auto 0', 'text-align': 'center', 'align-items': 'center', 'justify-content': 'center', overflow: 'scroll',
            ...
            <div style={{ 'text-wrap': 'balance', margin: '3%' }}>
                <p class='gold' style={{ 'z-index': 1, 'font-size': '0.6em' }}>{combatHistory()}</p>
                <div class='center creature-heading'>
                {combat().combatTimer && <p>Combat Timer: {combat().combatTimer}</p>}
                </div>
            </div> 
