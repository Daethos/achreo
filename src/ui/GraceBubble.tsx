...
export default function GraceBubble({ grace, show, setShow, settings }: {grace:Accessor<number>; show:Accessor<boolean>; setShow:Setter<boolean>; settings: Accessor<Settings>;}) {
    ...
    return <div class='graceBubble' onClick={() => setShow(!show())}>
        ...
        <p class='grace' style={{ 'margin-top': settings().grace === 'PERCENTAGE' ? '22.5%' : '20%', 'color': '#fdf6d8', 'font-size': settings().grace === 'PERCENTAGE' ? '1.15em' : '1.25em', 'font-weight': 'bold', 'text-shadow': '0.1em 0.1em 0.1em #000' }}>{settings().grace === 'NUMBER' ? newGrace() : settings().grace === 'PERCENTAGE' ? `${Math.round(gracePercentage())}` : ''}{ settings().grace === 'PERCENTAGE' && <span class='super' style={{ 'font-size': '0.5em' }}>%</span>}</p>
    </div>;
};