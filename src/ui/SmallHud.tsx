...
export default function SmallHud({ ascean, asceanState, combat, game, settings, quests, reputation, instance }: Props) { 
    ...
    return <>
       ...
       <Show when={game().showCombat}>
            <button class="highlight" onClick={() => setEditTextShow(!editTextShow())} style={{ top: `${Number(settings().combatText.top.split("vh")[0]) - 12}vh`, left: `${Number(settings().combatText.left.split("vw")[0]) - 1.25}vw`, position: "absolute", color: "gold", transform: "scale(0.75)" }}>{svg("UI")}</button>
