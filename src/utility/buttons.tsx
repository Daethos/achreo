import { borderColor, font } from "./styling";
...
export function PrayerModal({ prayer, show, setShow }: { prayer: Accessor<StatusEffect>, show: Accessor<boolean>, setShow: Setter<boolean> }) {
    ...
    return (
        <div class="modal" onClick={() => setShow(!show)}>
            <button class="thick-border superCenter" onClick={() => setShow(!show)} style={{ "border-color": `${borderColor(prayer()?.prayer)}`, "max-height": "85%", "max-width": "35%","overflow-y": "scroll", "overflow-x": "hidden" }}>
                <div class="creature-heading" style={{ height: "100%", margin: "5%" }}>
                    ...
                    <div style={{ color: "#fdf6d8" }}>
                        Prayer: <span class="gold">{prayer()?.prayer}</span> <br /><br />
                        Duration: <span class="gold">{prayer().duration}</span> <br />
                        Start: <span class="gold">{prayer()?.startTime}s</span> | End: <span class="gold">{prayer()?.endTime}s</span>
                    <div>
                </div>
                {prayer()?.refreshes ? 
                    <>Active Refreshes: <span class="gold">{prayer()?.activeRefreshes}</span></> : 
                    <>Active Stacks: <span class="gold">{prayer()?.activeStacks}</span></>}
                <div>
                <br />
                </div>
                ...
                {prayer()?.effect?.magicalDamage && 
                    <div>Magical Damage: <span class="gold">{prayer()?.effect?.magicalDamage}</span><br /></div>
                }
                {prayer()?.effect?.physicalPenetration && 
                    <div>Physical Penetration: <span class="gold">{prayer()?.effect?.physicalPenetration}</span><br /></div>
                }
                {prayer()?.effect?.magicalPenetration && 
                    <div>Magical Penetration: <span class="gold">{prayer()?.effect?.magicalPenetration}</span><br /></div>
                }
                {prayer()?.effect?.criticalChance && 
                    <div>Critical Chance: <span class="gold">{prayer()?.effect?.criticalChance}</span><br /></div>
                }
                {prayer()?.effect?.criticalDamage && 
                    <div>Critical Damage: <span class="gold">{prayer()?.effect?.criticalDamage}</span><br /></div>
                }
                {prayer()?.effect?.physicalPosture && 
                    <div>Physical Posture: <span class="gold">{prayer()?.effect.physicalPosture}</span><br /></div>
                }
                {prayer()?.effect?.magicalPosture && 
                    <div>Magical Posture: <span class="gold">{prayer()?.effect.magicalPosture}</span><br /></div>
                }
                {prayer()?.effect?.physicalDefenseModifier && 
                    <div>Physical Defense Modifier: <span class="gold">{prayer()?.effect?.physicalDefenseModifier}</span><br /></div>
                }
                {prayer()?.effect?.magicalDefenseModifier && 
                    <div>Magical Defense Modifier: <span class="gold">{prayer()?.effect?.magicalDefenseModifier}</span><br /></div>
                }
                {prayer()?.effect?.roll && 
                    <div>Roll Chance: <span class="gold">{prayer()?.effect?.roll}</span><br /> </div>
                }
                {prayer()?.effect?.dodge && 
                    <div>Dodge Distance: <span class="gold">{prayer()?.effect?.dodge}</span><br /> </div>
                }
                {prayer()?.effect?.healing && 
                    <div>Healing: <span class="gold">{Math.round(prayer()?.effect?.healing as number)}</span><br /> </div>
                }
                {prayer()?.effect?.damage && 
                    <div>Damage: <span class="gold">{Math.round(prayer()?.effect?.damage as number)}</span><br /> </div>
                }
                </div>
            </div>
            </button>
            <button class="cornerBR highlight" onClick={() => consume()} style={{ color: "red", "font-weight": 900 }}>
                Consume
            </button>
        </div>
    );
