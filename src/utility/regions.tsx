...
export const institutions: Institutions = {
    ...
    Museum: {
        preamble: `"To others within the Museum, it is ineffectual to hail everyone as Sage, for there are rungs to be climbed. To take this for arrogance or insolence though, as there are many Sages bustling about, and you may never see one’s eyes or veritable features enough to recall them by name. Nor in this respect is it likely to remember their title, however, it is fairly easy to track given their raiment worn. This is standard practice for the Neophytes as they are by design, novice in their understanding. Over time, the level of familiarity will grow, and mentorships will be forged, leaving some to be preferred by their given name, and others more stringent in their adherence to the tradition and always have their name preceded by their ranking. However, to the outside world, the common folk, and those not a part of their society, collectively they refer to the members as Sages, formally. Again, over time this may change and shift due to the nature of respect, yet it remains the most common and distinguishable euphemism."`,
...
export const whispers: Whispers = { // Localized Provincial Lore
    ...
    Draochre: {
        history:`"The Dachreon were forged in times long since past, during the age of the Ancients. While it’s unclear as to how many of the Ancients they worshiped in addition to nature, scholars argue whether the Primal Ancients were adhered to in addition to Achreo and Cambire, who had more scrolls and tomes attributed to their reverence. The Dachreon had a role to play in the Last Rites of the Ancients, giving themselves over to Achreo in order to restore the lands from breaking and sundering. To this day, Licivitas has adopted a variant of these beliefs in their institution of Arbiters, paying homage to the ways of old. \n
...
export const WhispersButtons = ({ current, options, handleConcept, handleWhisper }: { current: any, options: any, handleConcept: any, handleWhisper: any }) => {
    const buttons = Object.keys(options).map((o: any) => {
        const text = o.split("_").join(" ");
        return <div style={{ margin: "5%"}}>
